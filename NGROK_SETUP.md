...
const ContentView: React.FC<ContentViewProps> = ({
  ...
  const handlePlayVideo = async (id: string) => {
    ...
    // Only auto fullscreen for PWA (not browser)
    if (isMobile() && isPWA()) {
      try {
        // Request fullscreen after a short delay to ensure video loads
        setTimeout(async () => {
          const videoElement = document.querySelector('.video-card iframe') as HTMLElement;
          if (videoElement && videoElement.requestFullscreen) {
            try {
              await videoElement.requestFullscreen();
              // Show landscape hint for PWA users
              if (window.innerHeight > window.innerWidth) {
                // Show a brief hint to rotate device (only if in portrait)
                const hint = document.createElement('div');
                hint.innerHTML = '🔄 Rotate to landscape for better viewing';
                hint.style.cssText = `
                  position: fixed;
                  top: 20px;
                  left: 50%;
              ...
              console.log('Fullscreen request failed, video will play normally');
            }
          }
        }, 500);
      } catch (error) {
        console.log('Video fullscreen setup failed, video will play normally');
      }
    }
  };

  // Handle video pause/stop to exit fullscreen on mobile PWA
