...
export const register = async (_req: Request, res: Response) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: newUser.id, email: newUser.email }, jwtsecret, { expiresIn: "5h" });
...
export const login = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: existingUser.id, email: existingUser.email }, jwtsecret, { expiresIn: "5h" });
...
export const resetPassword = async (req: Request, res: Response) => {
    try {
        ...
        const newToken = jwt.sign({ id: user.id, email: user.email }, jwtsecret, { expiresIn: "5h" });
        return res.json({
